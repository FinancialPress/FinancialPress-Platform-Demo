...
const EnhancedChart = ({
      ...
      return (
        <div
          className={`p-3 rounded-lg border shadow-lg backdrop-blur-sm ${
            isDarkMode ? 'bg-gray-900/90 border-gray-700 text-white' : 'bg-white/90 border-gray-300 text-black'
          }`}
        >
          <p className="text-sm font-medium">{label}</p>
          <p className="text-lg font-bold text-yellow-400">${data.value.toFixed(2)}</p>
          ...
          {showVolume && data.volume && (
            <p className="text-xs opacity-75">Volume: {(data.volume / 1000000).toFixed(1)}M</p>
          )}
        </div>
      );
    }
  ...
  const sidebarMetrics = [
    ...
    {
      label: "Day's Range",
      value: `${Math.min(...chartData.map((d) => d.value)).toFixed(2)} - ${Math.max(...chartData.map((d) => d.value)).toFixed(2)}`,
    },
  ...
  const secondaryTextClasses = isDarkMode ? 'text-gray-400' : 'text-gray-600';
  ...
  return (
    <div className={`relative w-full h-full ${className}`}>
      ...
      <div className="relative z-10 w-full h-full">
        {/* Wrapped header, timeframes, and chart inside Card */}
        <Card className={`${cardClasses} w-full mb-6`}>
          <CardContent className="p-6">
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="flex items-center space-x-2">
                  <span
                    className={`${textClasses} text-2xl font-bold ${isAnimating ? 'animate-pulse scale-105' : ''}`}
                  >
              ...
              {showFullscreen && (
                <button
                  ...
                  className={`inline-flex items-center justify-center h-9 px-3 rounded-md text-sm font-medium transition-colors border ${
            ...
            <div className="flex space-x-1 mb-6 overflow-x-auto">
              {timeframes.map((timeframe) => {
                ...
                return (
                  ...
                  >
                    {isSelected && <div className="absolute inset-0 bg-yellow-400/20 rounded-md animate-pulse" />}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 h-full">
              ...
              <div className={`${chartAreaColSpan} min-w-0`}>
                ...
                {showVolume && (
                  <div
                    className={`mt-4 rounded-lg border overflow-hidden ${cardClasses}`}
                    style={{ height: '100px' }}
                  >
                    <ResponsiveContainer width="100%" height="100%">
