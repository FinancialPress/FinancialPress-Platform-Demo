...
const OnboardingFlow = ({ userRole = 'creator', onComplete, onLandingPage }: OnboardingFlowProps) => {
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      case 1:
        return <SocialChannelConnection onContinue={() => setCurrentStep(2)} />;
      case 2:
        return (
          <TopicSelection
            userRole={userRole}
      ...
      case 4:
        return <OnboardingWelcome userRole={userRole} onComplete={handleComplete} />;
      default:
        return <SocialChannelConnection onContinue={() => setCurrentStep(2)} />;
    }
  };
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-black text-white' : 'bg-white text-black'} relative`}>
      <button
        className={`absolute top-6 right-8 z-20 rounded-full p-2 transition-colors ${
        ...
        aria-label="Close"
      >
        <X className={`w-6 h-6 ${isDarkMode ? 'text-gray-300' : 'text-black'}`} />
      </button>
